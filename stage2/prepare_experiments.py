...
from data.ctr_dataset_wjj import Avazu2party, AvazuAug2party
...
def get_dataset(args):
    ...
    if args.dataset == 'ctr_avazu2party':
        ...
        if args.exp_type == 'cls':
            ...
            # save dataset for FATE
            if False:
                import pandas as pd
                print(test_dataset.x[0].shape) # (57511, 70) 有标签
                print(test_dataset.x[1].shape) # (57511, 50)
                print(test_dataset.y.shape) # (57511,)
                # FATE需要guest有标签
                pd.DataFrame(np.concatenate((train_dataset.y.reshape(-1, 1), train_dataset.x[0]), axis=1), columns=['y',]+['x'+str(i) for i in range(train_dataset.x[0].shape[-1])]).to_csv('train_host.csv', index=True, index_label='id')
                pd.DataFrame(train_dataset.x[1], columns=['x'+str(i) for i in range(train_dataset.x[1].shape[-1])]).to_csv('train_guest.csv', index=True, index_label='id')
                pd.DataFrame(np.concatenate((test_dataset.y.reshape(-1, 1), test_dataset.x[0]), axis=1), columns=['y',]+['x'+str(i) for i in range(test_dataset.x[0].shape[-1])]).to_csv('test_host.csv', index=True, index_label='id')
                pd.DataFrame(test_dataset.x[1], columns=['x'+str(i) for i in range(test_dataset.x[1].shape[-1])]).to_csv('test_guest.csv', index=True, index_label='id')
                
                print('feature_list', train_dataset_aug.feature_list)
                import pickle
                with open('feature_list.data', 'wb') as f:
                    pickle.dump(train_dataset_aug.feature_list, f)

            # print('len(test_dataset)',len(test_dataset), len(test_dataset_aug)) # 61503 40000
...
def get_model(args):
    ...
    if args.model == 'resnet':
        ...
        for i in range(args.k):
            ...
            # print('args.col_names', i, args.col_names[args.client_idx[i]])
